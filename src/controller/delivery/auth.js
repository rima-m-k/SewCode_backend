...
async function deliveryLogin(req, res, next) {
  try {
...
async function deliveryRegister(req, res, next) {
    try {
        ...
        let checkUser = await DELIVERY.findOne({ email: email });
        if (checkUser) {
...
module.exports = { deliveryLogin,deliveryRegister }
